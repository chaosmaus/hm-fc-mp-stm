  ...
  const filterController = (data) => {
    ...
    if ($(".item.hidden").length === $(".item").length)
      ...
      $(".locations-cms_item").each((index, element) => {
        ...
        availableProperties.forEach((el, i) => {
          if( $(element).find('.location-field_guesty').text() === el.listingId ){
            $(element).find(".location-field_lang").text(el.address.lat)
            $(element).find(".location-field_lat").text(el.address.lng)
          }
        });
  ...
  const loadData = () => {
    $(".locations-cms_item").each((index, element) => {
      if ($(`.item:eq(${index})`).hasClass("hidden")) {
        ...
        if(!(Number($(element).find(".location-field_lat").text()) < 90) && !(Number($(element).find(".location-field_lat").text()) > -90)){
          console.log('wrong coordintates for this property: ', $(element).find(".location-field_new-name").text())
        }
        ...
        geoData.push({
          ...
          properties: {
            ...
            title: $(element).find(".location-field_new-name").text(),
