$(document).ready(function () {
        ...
        map.on("click", "unclustered-point", function (e) {
          ...
          const popup = new mapboxgl.Popup({ closeOnClick: false })
            ...
            $('.items').each((index, element) => {
              if( $(element).find('.item_guesty-id') === e.features[0].properties.guesty){
    ...
    /* FEED MOBILE CARD CONTROLLER */
    $('.map_mobile-card_close-button').on('click', (e)=>{
      let clickedButton = $(e.target);
      if(!($('.map_mobile-card_wrapper').hasClass('hidden'))){
        if(clickedButton.is('.map_mobile-card_close-button_icon')) clickedButton.parent().parent().parent().addClass('hidden')
        else clickedButton.parent().parent().addClass('hidden')
      }
    })
